...
function main(param) {
    ...
    scene.onLoad.add(function () {
        // 以下、webカメラからの描画
        var scale = 8;
        var cameraShotWidth = g.game.width / scale;
        var cameraShotHeight = g.game.height / scale;
        ...
        var matSrc = null;
        var matDst = null;
        // documentを利用するためこの条件分岐が無いとサーバー側で死ぬ
        // TODO: 条件分岐が要らなくなるようになんとかしたい
        if (cv) {
        ...
        scene.onUpdate.add(() => {
            ...
            const hands = detectHands(matSrc, handCascade);
            hands.forEach(hand => {
            ...
            if (hands.length === 0) {
                ...
                player.show();
                const place = hands[0];
                player.x = (place.x + place.width / 2) * scale;
                player.y = (place.y + place.height / 2) * scale;
                player.modified();
                shot(scene, player, shotImageAsset, seAudioAsset);
            }
        });
        // ここまでゲーム内容を記述します
    });
...
function shot(scene, player, shotImageAsset, seAudioAsset) {
    seAudioAsset.play();
    // プレイヤーが発射する弾を生成します
